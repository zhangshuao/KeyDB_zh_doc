...
KeyDB加载默认配置文件，如下所示。如果指定配置文件，则可以自定义以快速重复加载。keydb-server /path-to-custom-conf/keydb.conf

当运行多个服务器、副本等时，最好自定义配置文件，并至少指定以下命令以保持独立：
...
KeyDB可以在没有使用内置默认配置的配置文件的情况下启动，但是建议仅在测试和开发时使用此设置。
配置KeyDB的正确方法是提供一个配置文件，通常称为 keydb.conf 或 redis.conf。
keydb.conf文件包含许多格式非常简单的指令:
...
可以使用引号提供包含空格的字符串作为参数，如下例所示：

    requirepass "hello world"

## 通过命令行传递参数

也可以直接使用命令行传递配置参数。这对于测试非常有用。下面的示例使用端口6380作为在127.0.0.1端口6379上运行的实例的从机来启动新的KeyDB实例。

    ./keydb-server --port 6380 --slaveof 127.0.0.1 6379

通过命令行传递的参数的格式与keydb.conf文件中使用的参数的格式完全相同，只是关键字的前缀是 --。

注意，在内部，这会生成一个内存中的临时配置文件（如果有的话，可能会连接用户传递的配置文件），其中的参数被转换为keydb.conf格式。

## 在服务器运行时更改KeyDB配置

可以在不停止和重新启动服务的情况下动态重新配置KeyDB，也可以使用特殊命令CONFIG SET 和 CONFIG GET 以编程方式查询当前配置

并非所有配置指令都以这种方式受支持，但大多数指令都按预期受支持。有关详细信息，请参阅配置集和配置获取页面。

请注意，动态修改配置**对keydb.conf文件没有影响**，因此在下一次重新启动Redis时将使用旧配置。

确保还根据使用配置集设置的配置修改keydb.conf文件。您可以手动执行，也可以只使用CONFIG REWRITE，它将自动扫描keydb.conf文件并更新与当前配置值不匹配的字段。
未添加但设置为默认值的字段不被添加。将保留配置文件中的注释。

## 将KeyDB配置为缓存

如果计划将KeyDB用作缓存，其中每个密钥都有一个expire集，则可以考虑改用以下配置（以最大内存限制为2兆字节为例）：

    maxmemory 2mb
    maxmemory-policy allkeys-lru
    
在这种配置中，应用程序不需要为使用到期命令（或等效）的密钥设置生存时间，因为只要我们达到2兆字节的内存限制，所有key都将使用近似的LRU算法来驱逐。

在这个配置中，KeyDB的行为与memcached类似。在"命令"部分，我们有更多关于使用Redis作为LRU缓存的文档。

请参见下面的默认keydb配置文件（当前标记为keydb.conf）。它解释了配置参数。
